...
This document describes the AMS template chip from a designer's point of view. It tells you what
the chip contains, how it behaves, and where each subsystem is configured. The numbers below are
read from the design files. If you change a number in the source, write the new value here also.
...
The six corner names are the same as on the SG13G2 sibling of this template. They resolve to
`sg13cmos5l_stdcell` Liberty files, plus the IO Liberty files from
[ip/sg13cmos5l_io_custom/lib/](../ip/sg13cmos5l_io_custom/lib/).


## Padframe layout (8 × 8 = 32 pads)

The padframe uses the generic top-level port names defined in [chip_top.sv](../rtl/chip_top.sv).
[chip_core.sv](../rtl/chip_core.sv) maps the bit indices to roles.
...
[pinout.md](pinout.md) gives the full per-pad breakdown, including the pad-cell instance names.
...
[floorplan.md](floorplan.md) gives the exact coordinates, the floorplan diagram and the overlap
rules.
...
`input_PAD[0]` is the chip global enable signal. [chip_core.sv](../rtl/chip_core.sv) also names
it `enable`. It gates:
...
`counter1` and `counter2` are two instances of the `counter_top` macro (8-bit, default
`CTR_MAX = 255`). They share `clk_PAD`, `rst_n_PAD` and `input_PAD[0]`:
...
LibreLane hardens the counter on `sg13cmos5l_stdcell`. The PDK default routing window is Metal2
to TopMetal1. [macros/counter/flow/librelane/config.yaml](../macros/counter/flow/librelane/config.yaml)
carries a commented-out `RT_MAX_LAYER: Metal4`. That line caps the macro below the chip's
TopMetal1 PDN, if a future floorplan needs it.
...
`inverter2` (instance of `inverter_top`) is used as an analog block. Four east-side analog pads
expose two of its four channels:
...
Channels 3 and 4 (`vin3`, `vout3`, `vin4`, `vout4`) are tied to `VSS` inside
[chip_core.sv](../rtl/chip_core.sv) and are not bonded out.

The `padres` path goes through the IO pad's secondary ESD resistor. The `padbare` path bypasses
it. That gives lower parasitics, and the output signals use it.

`RSZ_DONT_TOUCH_LIST` in [config.yaml](../flow/librelane/config.yaml) lists the four nets, so the
resizer leaves them alone. During detailed routing, the `DRT_ASSIGN_NDR` regex applies the wider
`NDR_analog` non-default rule to them. `NDR_analog` covers Metal1 to Metal4 and the double-cut
vias `Via1_DC1B`, `Via2_DC1B` and `Via3_DC1B`. The SG13G2 version of the same rule has a Metal5
entry and a fourth via. This stack has neither.

The `inverter_top` macro is a hand-drawn layout. It was redrawn for the SG13CMOS5L design rules,
at the same 54.18 × 81.92 µm footprint as its SG13G2 counterpart. See
[macros/inverter/README.md](../macros/inverter/README.md) for its own build and sign-off flow.
That README also notes that KPEX has no SG13CMOS5L support yet, so Magic is the extraction path
here.

### SRAM

[chip_core.sv](../rtl/chip_core.sv) wires the `RM_IHPSG13_1P_1024x32_c2_bm_bist` macro up for a
simple read-only demonstration:
...
The behavioural model has no `USE_POWER_PINS` block. The Verilog instantiation therefore connects
only the functional and BIST ports. The PDN level makes the physical supply connection, through
`PDN_MACRO_CONNECTIONS` in [config.yaml](../flow/librelane/config.yaml). That key binds the
macro's `VDD!/VSS!` and its secondary `VDDARRAY!/VSS!` to the chip's `VDD`/`VSS`.

The IHP SRAM macros are not characterised for every corner of this template, and in particular
not above 1.32 V. The `lib` block of the macro in
[config.yaml](../flow/librelane/config.yaml) therefore reuses
`RM_IHPSG13_1P_1024x32_c2_bm_bist_fast_1p32V_m55C.lib` for the three corners that have no
matching Liberty. Those three corners are `nom_fast_1p65V_m40C`, `nom_slow_1p35V_125C` and
`nom_typ_1p50V_25C`. The substitution is deliberate. `nom_typ_1p50V_25C` is also the default
corner. Any SRAM timing number that the flow reports at the default corner is therefore read from
a 1.32 V / -55 °C model.
...
- Each `inverter_top` instance has its own internal power ring, 2.2 µm wide. Its horizontal segments are on Metal3 and its vertical segments are on Metal4. The chip's TopMetal1 stripes cross this ring and feed it through the macro grid in [pdn_cfg.tcl](../flow/librelane/pdn_cfg.tcl).
- The SRAM's supply pins are vertical Metal4 stripes, which is the chip's own vertical PDN layer. Its grid (`sram` in [pdn_cfg.tcl](../flow/librelane/pdn_cfg.tcl)) is therefore a plain Metal4 to TopMetal1 connect with no extra stripes.
- Active-low reset on `rst_n_PAD`. The counter macro converts the polarity internally and feeds a synchronous active-high reset to its FFs.
