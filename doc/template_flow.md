# Template flow reference

This file holds the flow documentation that came from the README of the upstream
...
- The PDK pin, which sets `ihp-sg13cmos5l` in every Makefile and every `xschemrc`.

Read the upstream README or the
[upstream tutorial](https://iic-jku.github.io/ihp-sg13cmos5l-ams-chip-template/index.html)
for the template without these changes.
...
│     └─ chip_logo_mono.png
├─ 📁 ip/
│  ├─ 📁 sg13cmos5l_io_custom/
│  ├─ 📁 sg13cmos5l_ip__bondpad_70x70/
│  │  ├─ 📁 final/
│  │  ├─ 📁 script/
│  │  ├─ 📁 verification/
│  │  ├─ Makefile
...
- `chip_top_black_TM1.png`: only `TopMetal1`, `TopVia1`, the `TopMetal1` filler and `Passiv` on a black background, which shows the chip logo and the padframe.
...
```sh
make init-submodule MACRO=<macro> SUB=<subcell> [TYPE=analog|digital]
```
...
- `render/img/chip_top_black_TM1.png` -> `release/v.<VERSION>/img/chip_top_black_TM1.png`
...
| PNG to GDS logo generation, KLayout DRC, Magic DRC | `sg13cmos5l_ip__jku all` (single logo) |
...
| KLayout DRC (`sak-drc.sh`) + KLayout LVS (`sak-lvs.sh`) | inverter `klayout-verify CELL=inverter_top` |
...
- the bondplan outputs in `packaging/` (`render/`, `result.md`, and in `layout/` the generated `chip_top_bondplan.gds`, `chip_top_logo_TM1.gds.gz` and the extracted package footprint `OP_QFN32_A4_FIT.gds`)
